...
function setTab(tab) {
  ...
  // Hide editor picks when viewing academics
...
async function boot() {
  const [feedResp, acaResp] = await Promise.all([ fetch(FEED_URL), fetch(ACADEMICS_URL) ]);
  items = await feedResp.json();
  academics = acaResp.ok ? await acaResp.json() : [];

  // Editor picks (3) – prefilled below
  const picks = window.EDITOR_PICKS || [];
  if (dom.editorGrid && picks.length) {
    // Editor picks layout: Title, teaser (3 lines via CSS clamp already), then "Como usou a LAI?" label and link
    const renderPick = (p) => {
