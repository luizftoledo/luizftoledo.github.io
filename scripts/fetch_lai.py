...
# Data sources
GOOGLE_ALERTS_URLS = [
    "https://www.google.com/alerts/feeds/11574052472963582697/4117947282941867830",  # geral LAI na imprensa
    "https://www.google.com/alerts/feeds/11574052472963582697/6650523041459024237",  # LAI acadêmica (Scholar e periódicos)
]
INOREADER_SOURCES = [
    {
    ...
    # Você pode adicionar aqui novos streams públicos do Inoreader e definir force_category="academic" se desejar
]

ROOT = Path(__file__).resolve().parents[1]
# Allow overriding the output directory via env var (relative to repo root)
...
OVERRIDES = ROOT / "a_lai_que_pegou" / "data" / "overrides.json"
...
class InoreaderHTMLParser(HTMLParser):
    """Very lightweight parser to extract external article links and titles from Inoreader public HTML stream."""
...
def parse_inoreader_html(html_bytes: bytes):
    ...
    for link in parser.links:
        ...
        entries.append({
            ...
            "url": url,
            "summary": "",
            # No reliable per-item timestamp in this simple parse; use now
...
def main():
    ...
    # Try Inoreader first (public HTML streams)
    ...
    # Fallback: Google Alerts Atom (may be empty/private)
    ...
    # Split into news (journalistic) and academics only
    ...
    # Convert academics to academics.json schema expected by the UI
    def to_academic(it):
        year = None
        try:
    ...
    print(
        f"Wrote {len(news_like)} news/tutorial/gov items to {OUT} and {len(academics)} academic items to {OUT_ACADEMICS}"
    )
